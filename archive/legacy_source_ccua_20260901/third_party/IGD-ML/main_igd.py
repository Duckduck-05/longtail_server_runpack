...
def train():
    ...
    writer.flush()
    wandb.init(project="longtail-baselines", name="igd", config=FLAGS.flag_values_dict(), resume="allow")
        ...
        for step in pbar:
            if FLAGS.igd:
                ...
                for y in range(FLAGS.num_class):
                    ...
                    x_0 = x_0.to(device)
                    y_0 = y_0.to(device)
                    _t = trainer.module if hasattr(trainer, 'module') else trainer
                    loss_y = _t.compute_per_class_loss(x_0, y_0, augm)
                    loss_y.backward()
