...
flags.DEFINE_integer('ddim_steps', 0, help='ltx_coral_ddim_v1: if >0, sample with this many DDIM steps instead of the full T-step ancestral chain')
...
flags.DEFINE_enum('ipsvt_sampler', 'uniform', ['uniform', 'sqrt'],
                  help='auxiliary class sampler: uniform or proportional to sqrt(class support)')
flags.DEFINE_integer('ipsvt_warmup', 0,
                     help='do not apply the auxiliary branch before this global training step')
...
def ema(source, target, decay):
    ...
    for key in source_dict.keys():
        target_dict[key.replace("_orig_mod.", "")].data.copy_(
            target_dict[key.replace("_orig_mod.", "")].data * decay +
...
def train():
    ...
    if FLAGS.ipsvt:
        ...
        ipsvt_aux_branch = IPSVTAuxiliary(
            ...
            sampler=FLAGS.ipsvt_sampler, warmup_steps=FLAGS.ipsvt_warmup)
        del _raw
    ...
    with trange(FLAGS.ckpt_step, FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            wandb.log({"loss": loss.item(), "loss_ddpm": loss_ddpm.item(),
                       "loss_supcon": loss_supcon.item() if FLAGS.supcon else 0}, step=step)
...
def eval():
    ...
    sampler = GaussianDiffusionSampler(
        model, FLAGS.beta_1, FLAGS.beta_T, FLAGS.T, FLAGS.num_class, FLAGS.img_size, FLAGS.var_type,
