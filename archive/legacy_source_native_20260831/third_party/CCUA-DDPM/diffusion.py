...
class GaussianDiffusionSamplerDDIM(nn.Module):
    ...
    def p_mean_variance(self, x_t, t, y, method, skip):
        ...
        if self.mean_type == 'xprev':       # the model predicts x_{t-1}
            ...
            if self.cond:
                eps, embed = self.model(x_t, t, y, return_mid=True)
                eps_g, embed_g = self.model(x_t, t, None, return_mid=True)

                eps = eps + (self.omega_t) * (eps - eps_g)
                embed = embed + (self.omega_t) * (embed - embed_g)
        ...
        return model_mean, model_log_var, x_0, embed
    ...
    def forward(self, x_T, y, method='ddim', skip=10, return_timestep_status=False):
        """
        Algorithm 2.
        ...
        for time_step in tqdm(reversed(range(0, self.T, skip)), total=self.T//skip):
            ...
            mean, log_var, x0_pred, embed = self.p_mean_variance(x_t=x_t, t=t, y=y, method=method, skip=skip)
            # no noise when t == 0
            if time_step > 0:
        ...
        if return_timestep_status:
            ...
            return (torch.clip(x_0, -1, 1),
                    torch.stack(xt_list).permute(1,0,2,3,4),
                    torch.stack(embed_list).permute(1,0,2,3,4),
                    torch.stack(x0_pred_list).permute(1,0,2,3,4),
                    torch.tensor(omega_t_list).to(x_t.device))
...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def forward(self, x_0, y_0):
        ...
        eps, latent = self.model(x_t, t, y=y_l, return_mid=True)
        if self.transfer_x0:
        ...
        loss_cbdm = torch.zeros(eps.shape).to(x_t.device)
        if self.cbdm and y_l is not None:
            y_bal = torch.Tensor(np.random.choice(
                self.num_class, size=len(x_0),
                p=self.weight.squeeze(0).numpy(),
            )).to(x_t.device).long()
            eps_bal = self.model(x_t, t, y=y_bal, return_mid=False)
            weight = t[:, None, None, None] / self.T * self.cb_tau
            loss_reg = weight * F.mse_loss(eps, eps_bal.detach(), reduction='none')
            loss_com = weight * F.mse_loss(eps.detach(), eps_bal, reduction='none')
            loss_cbdm = loss_reg + 1 / 4 * loss_com

        loss_ccua_ucl = torch.zeros(x_t.shape[0]).to(x_t.device)
        loss_ccua_al = torch.zeros_like(eps).to(x_t.device)
        # adaptive_weight = lambda t, tau: (t / self.T) ** tau / ((t/self.T) ** tau + (1-t/self.T) ** tau)
        adaptive_weight = lambda t, tau: (t / self.T) ** tau
        if (self.ccua_ucl > 0.0 or self.ccua_al > 0.0):
            eps_unc, latent_unc = self.model(x_t, t, y=None, return_mid=True)
            if self.ccua_ucl > 0.0:
                queries, keys = latent_unc, latent_unc
                adaptw = adaptive_weight(t, tau=1.0)
                loss_ccua_ucl = adaptw * uncond_info_nce(queries, keys, reduction='none')
            if self.ccua_al > 0.0 and y_l is not None:
                adaptw = adaptive_weight(t, tau=1.0)
