...
def main() -> None:
    ...
    parser.add_argument("--repo", required=True, help="vendored ImbDiff-CM root containing FID Inception weights")
    ...
    images, labels = load_generated(
        Path(args.samples).resolve(), Path(args.labels).resolve(), args.num_images, args.image_size, args.num_classes
    )
    weights = repo / "stats" / "pt_inception-2015-12-05-6726825d.pth"
    ...
    payload = {
        "protocol": "ImageNet-LT 64x64 secondary: FID/KID, released CM FID-Inception, generated balanced grid",
        "FID": fid,
