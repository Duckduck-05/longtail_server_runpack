...
class GaussianDiffusionSamplerOldCMBase(nn.Module):
    ...
    def p_mean_variance(self, x_t, t, y,method, skip):
        ...
        if self.mean_type == 'xprev':       # the model predicts x_{t-1}
            x_prev = self.model(x_t, t, y, use_cm=False)
            x_0 = self.predict_xstart_from_xprev(x_t, t, xprev=x_prev)
            model_mean = x_prev
        elif self.mean_type == 'xstart':    # the model predicts x_0
            x_0 = self.model(x_t, t ,y, use_cm=False)
            model_mean, _ = self.q_mean_variance(x_0, x_t, t)
        elif self.mean_type == 'epsilon':   # the model predicts epsilon
            if self.cond:
                eps = self.model(x_t, t ,y, use_cm=False)
                eps_g=self.model(x_t, t ,None, use_cm=False)
                eps=eps+(self.w)*(eps-eps_g)
                x_0 = self.predict_xstart_from_eps(x_t, t, eps=eps)
                model_mean, _ = self.q_mean_variance(x_0, x_t, t, method, skip, eps)
            else:
                eps = self.model(x_t, t, use_cm=False)
                x_0 = self.predict_xstart_from_eps(x_t, t, eps=eps)
                model_mean, _ = self.q_mean_variance(x_0, x_t, t, method, skip, eps)
...
class GaussianDiffusionMid(nn.Module):
    ...
    def forward(self, x_0, y_0, augm=None,fix_t=None):
        ...
        if fix_t is None:
            ...
            t = torch.full((x_0.shape[0], ), fix_t, device=x_0.device, dtype=torch.long)
...
class GaussianDiffusionTrainer(nn.Module):
    def __init__(self,
                 ...
                 double_transfer=False, w_con = 1., w_div = 1.):
        super().__init__()
        ...
        self.inverse_weight = 1/self.weight
        self.inverse_weight = self.inverse_weight / self.inverse_weight.sum()
        self.transfer_x0 = transfer_x0
    ...
    def forward(self, x_0, y_0, augm=None,uncond_flag_out=False):
        ...
        h1 = self.model(x_t, t, y=y_l, augm=augm, use_cm=True)
        h2 = self.model(x_t, t, y=y_l, augm=augm, use_cm=False)
        if self.transfer_x0:
        ...
        loss1 = F.mse_loss(h1, noise, reduction='none').mean()
        # print(self.weight.device)
        # print(y_0.device)
        self.weight = self.weight.to(y_0.device)
        self.inverse_weight = self.inverse_weight.to(y_0.device)
        # print(self.weight.shape)
        # print(y_0.shape)
        # print(y_0)
        m_list_per_sample = self.weight[y_0]
        inverse_m_list_per_sample = self.inverse_weight[y_0]
        weight_div_per_sample = (m_list_per_sample* self.w_con - inverse_m_list_per_sample * self.w_div)
        loss2 = F.mse_loss(h2.view(h2.shape[0],-1), h1.view(h1.shape[0],-1), reduction='none').mean(dim=1)
        loss2 = (loss2 * weight_div_per_sample).mean() * self.num_class
    ...
    def do_transfer_x0(self,x_t,cx_t,x_0,t,y,return_transfer_label=False,mode=None,x_ref=None):
        ...
        with torch.no_grad():
            ...
            new_ind = new_ind.squeeze()
            ini_ind = torch.arange(x_0.shape[0], device=x_0.device)
            transfer_label = y[new_ind]
            class_weight = self.weight.squeeze().to(x_0.device)
            old_prob = class_weight.gather(0, y)
            new_prob = class_weight.gather(0, transfer_label)
        ...
        for time_step in reversed(range(0, self.T,skip)):
            ...
            if method == 'ddim':
                ...
                if cond_fn is not None:
                    pred_x0, _ = self.condition_score(cond_fn, pred_x0, mean, t, y,method='ddpm',skip=skip)
