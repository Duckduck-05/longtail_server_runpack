...
def forward_with_rows(model, x: Tensor, t: Tensor, e_rows: Tensor) -> Tensor:
    ...
    """
    from model.model import ResBlock as T2HResBlock
    try:
        from model.model_cm import ResBlock as CMResBlock
        from model.model_cm import ResBlock_CM
        resblock_types = (T2HResBlock, CMResBlock, ResBlock_CM)
    except ImportError:  # pragma: no cover - native OC_LT-only fallback
        resblock_types = (T2HResBlock,)
    ...
    for layer in model.upblocks:
        if isinstance(layer, resblock_types):
