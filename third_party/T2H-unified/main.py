...
flags.DEFINE_string('resume_ckpt','./',help='the resumed checkpoint directory')
flags.DEFINE_integer('ckpt_step', 0, help='checkpoint step to resume from')
...
def train():
    ...
    writer.flush()
    wandb.init(project="longtail-baselines", name="oclt", config=FLAGS.flag_values_dict(), resume="allow")
    ...
    with trange(FLAGS.ckpt_step, FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            wandb.log({"loss": loss.item(), "loss_ddpm": loss_ddpm.item()}, step=step)
